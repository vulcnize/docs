...
title: "vulcn crawl"
seo:
  title: "vulcn crawl — Auto-Discover Forms and Injection Points"
...
This is essential for **automated benchmarking** — point it at a target, let it discover attack surfaces, and optionally chain directly into scanning.
...
5. Optionally chains into `vulcn run` with `--run-after`
...
vulcn crawl http://localhost -d 3 --run-after xss sqli -r html
```
...
✔ Saved 3 session file(s) to ./sessions

📋 Generated Sessions

  1. 01-crawl-search-form-1-query.vulcn.yml
     Crawl: /search — form 1 (query) — 1 injectable input(s)

  2. 02-crawl-login-form-1-username-password.vulcn.yml
     Crawl: /login — form 1 (username, password) — 2 injectable input(s)

  3. 03-crawl-contact-form-1-name-email-message.vulcn.yml
     Crawl: /contact — form 1 (name, email, message) — 3 injectable input(s)
```
...
- [vulcn run](/cli/run) — Run security tests on generated sessions
- [vulcn record](/cli/record) — Manually record browser interactions
