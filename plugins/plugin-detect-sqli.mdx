...
seo:
  title: "@vulcn/plugin-detect-sqli — SQL Injection Detection Plugin"
...
```bash
vulcn run session.vulcn.yml -p sqli
```
...
- **Body length change** — if the response body changes by >50% (and >500 bytes) and contains SQL error patterns, it's flagged
...
- If response time exceeds the `timingThresholdMs` threshold, the SQL command executed — confirming injection

## Example Output

```bash
🚨 SQLi DETECTED (MySQL): MySQL syntax error
...
No configuration needed — just use `sqli` payloads and detection is active.
...
- [Payload Types](/cli/payloads) — Available payload types including `sqli`
- [@vulcn/plugin-detect-xss](/plugins/plugin-detect-xss) — XSS detection plugin
- [Plugin Hooks](/plugins/hooks) — All available plugin hooks
