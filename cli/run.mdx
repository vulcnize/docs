...
title: "vulcn run"
seo:
  title: "vulcn run — Execute Security Tests with Payload Injection"
...
Payloads are fetched from [PayloadsAllTheThings](https://github.com/swisskyrepo/PayloadsAllTheThings) — the largest community-curated collection of security payloads.

## Payload Types

| Short Name  | Full Name           | Description                                     |
| ----------- | ------------------- | ----------------------------------------------- |
| `xss`       | `xss`               | Cross-Site Scripting — script injection, events |
| `sqli`      | `sql-injection`     | SQL Injection — auth bypass, UNION, error-based |
| `xxe`       | `xxe`               | XML External Entity — file read, SSRF via XML   |
| `cmd`       | `command-injection` | OS Command Injection — shell execution          |
| `redirect`  | `open-redirect`     | Open Redirect — URL redirect to attacker domain |
| `traversal` | `path-traversal`    | Path Traversal — directory traversal            |

Additional aliases: `sql` → `sql-injection`, `command` → `command-injection`, `lfi` → `path-traversal`
...
Produces a [SARIF v2.1.0](https://docs.oasis-open.org/sarif/sarif/v2.1.0/sarif-v2.1.0.html) file with CWE mappings—upload directly to GitHub Code Scanning:
...
- **`@vulcn/plugin-detect-xss`** — Always loaded. Detects XSS by monitoring dialogs and console markers.
- **`@vulcn/plugin-detect-sqli`** — Auto-loaded when `sqli` payloads are used. Detects SQL injection via error patterns, timing, and response diffing.
