...
<Accordion title="💉 Payload Injection" icon="syringe">
  ...
  Built-in payloads cover XSS, SQLi, SSRF, XXE, and more. Supports PayloadBox
  integration for thousands of community payloads. ```bash vulcn run
  session.vulcn.yml --payload xss-basic sqli-basic # Use PayloadBox community
  payloads vulcn run session.vulcn.yml --payload payloadbox:xss ```
</Accordion>
...
<Steps>
  ...
  <Step title="Choose Payloads">
    Select from built-in payloads (XSS, SQLi, etc.), use PayloadBox community
    payloads, or load custom ones from files.
  </Step>
...
# Run with PayloadBox payloads and generate a report
