...
The report plugin generates branded, professional security reports at the end of each `vulcn run`. It supports four output formats and integrates seamlessly with CI/CD pipelines and GitHub Code Scanning.
...
// HTML report
const html = generateHtml({ session, result, generatedAt, engineVersion });

// JSON report
const json = generateJson(session, result, generatedAt, engineVersion);

// YAML report
const yaml = generateYaml(session, result, generatedAt, engineVersion);

// SARIF report
const sarif = generateSarif(session, result, generatedAt, engineVersion);
