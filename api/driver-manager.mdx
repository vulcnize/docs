---
title: "DriverManager"
description: "API reference for the DriverManager class"
---

## Overview

The `DriverManager` class handles driver loading, registration, and lifecycle. It's the central point for managing recording and running drivers in Vulcn.

## Import

```typescript
import { DriverManager } from "@vulcn/engine";
```

## Constructor

```typescript
const manager = new DriverManager();
```

Creates a new DriverManager with no drivers registered.

## Methods

### register

Register a driver with the manager.

```typescript
register(driver: VulcnDriver, source?: DriverSource): void
```

**Parameters:**

- `driver` - The driver to register
- `source` - Optional source type (`"npm"`, `"local"`, `"builtin"`)

**Example:**

```typescript
import browserDriver from "@vulcn/driver-browser";

manager.register(browserDriver);
```

<Info>
  The first registered driver automatically becomes the default driver.
</Info>

### load

Load a driver from npm or a local file path.

```typescript
async load(nameOrPath: string): Promise<void>
```

**Parameters:**

- `nameOrPath` - npm package name or file path

**Example:**

```typescript
// Load from npm
await manager.load("@vulcn/driver-browser");

// Load from local file
await manager.load("./my-custom-driver.js");
```

### get

Get a loaded driver by name.

```typescript
get(name: string): VulcnDriver | undefined
```

**Example:**

```typescript
const driver = manager.get("browser");
if (driver) {
  console.log(driver.version);
}
```

### getDefault

Get the current default driver.

```typescript
getDefault(): VulcnDriver | undefined
```

### setDefault

Set the default driver by name.

```typescript
setDefault(name: string): void
```

**Throws:** Error if driver is not registered.

### has

Check if a driver is registered.

```typescript
has(name: string): boolean
```

### list

Get all registered drivers.

```typescript
list(): LoadedDriver[]
```

**Returns:** Array of loaded driver info including source metadata.

### getForSession

Get the appropriate driver for a session.

```typescript
getForSession(session: Session): VulcnDriver
```

**Throws:** Error if the session's driver is not registered.

**Example:**

```typescript
const session = parseSession(yaml);
const driver = manager.getForSession(session);
console.log(`Using ${driver.name} driver`);
```

### startRecording

Start recording with a specific driver.

```typescript
async startRecording(
  driverName: string,
  config: Record<string, unknown>,
  options?: RecordOptions
): Promise<RecordingHandle>
```

**Parameters:**

- `driverName` - Name of the driver to use
- `config` - Driver-specific configuration
- `options` - Optional recording options

**Returns:** A `RecordingHandle` for controlling the recording.

**Example:**

```typescript
const handle = await manager.startRecording("browser", {
  startUrl: "https://example.com",
  browser: "chromium",
});

// Wait for user interaction...

const session = await handle.stop();
```

### crawl

Auto-crawl a URL to discover forms and generate sessions.

```typescript
async crawl(
  driverName: string,
  config: Record<string, unknown>,
  options?: CrawlOptions
): Promise<Session[]>
```

**Parameters:**

- `driverName` - Name of the driver to use (must support crawling)
- `config` - Driver-specific configuration (e.g., `startUrl`, `browser`)
- `options` - Crawl options (depth, page limits, etc.)

**Returns:** Array of `Session` objects, one per discovered injectable form.

**Throws:** Error if the driver doesn't support auto-crawl.

**Example:**

```typescript
const sessions = await manager.crawl(
  "browser",
  {
    startUrl: "https://example.com",
    browser: "chromium",
    headless: true,
  },
  {
    maxDepth: 2,
    maxPages: 20,
  },
);

console.log(`Discovered ${sessions.length} injectable forms`);

// Scan all sessions with a shared browser
const result = await manager.executeScan(sessions, pluginManager, {
  headless: true,
});
```

<Info>
  Not all drivers support `crawl()`. Currently only the browser driver
  implements auto-crawl. Calling `crawl()` on a driver that doesn't support it
  throws a clear error.
</Info>

### execute

Execute a single session with payloads (standalone mode — launches its own browser).

```typescript
async execute(
  session: Session,
  pluginManager: PluginManager,
  options?: RunOptions
): Promise<RunResult>
```

**Parameters:**

- `session` - The session to execute
- `pluginManager` - Plugin manager with payloads loaded
- `options` - Run options (headless, callbacks, etc.)

**Returns:** `RunResult` with findings and statistics.

**Example:**

```typescript
const result = await manager.execute(session, pluginManager, {
  headless: true,
  onFinding: (finding) => console.log("Found:", finding.title),
});

console.log(`Found ${result.findings.length} vulnerabilities`);
```

### executeScan

Execute multiple sessions in a single scan with a **persistent browser** instance. This is the recommended method for production scanning — it's faster (shared browser, goBack-based payload cycling) and fires scan-level plugin hooks (`onScanStart`, `onScanEnd`).

```typescript
async executeScan(
  sessions: Session[],
  pluginManager: PluginManager,
  options?: RunOptions
): Promise<RunResult>
```

**Parameters:**

- `sessions` - Array of sessions to execute
- `pluginManager` - Plugin manager with payloads loaded
- `options` - Run options (headless, callbacks, storage state, etc.)

**Returns:** Aggregated `RunResult` with findings from all sessions.

**Example:**

```typescript
// Crawl to discover forms
const sessions = await manager.crawl("browser", {
  startUrl: "https://example.com",
  headless: true,
});

// Scan all sessions with a shared browser
const result = await manager.executeScan(sessions, pluginManager, {
  headless: true,
  onFinding: (finding) => console.log(`🚨 ${finding.title}`),
});

console.log(`Scanned ${sessions.length} forms`);
console.log(
  `Found ${result.findings.length} vulnerabilities in ${result.duration}ms`,
);
```

<Info>
  `executeScan()` fires `onScanStart` and `onScanEnd` hooks (wrapping all
  sessions), plus `onRunStart`/`onRunEnd` for each individual session. Use
  `execute()` only when you need to test a single session in isolation.
</Info>

## Types

### VulcnDriver

```typescript
interface VulcnDriver {
  name: string;
  version: string;
  apiVersion?: number;
  description?: string;
  configSchema?: z.ZodSchema;
  stepTypes: string[];
  recorder: RecorderDriver;
  runner: RunnerDriver;
}
```

### LoadedDriver

```typescript
interface LoadedDriver {
  driver: VulcnDriver;
  source: DriverSource;
}

type DriverSource = "npm" | "local" | "builtin";
```

### RecordingHandle

```typescript
interface RecordingHandle {
  stop(): Promise<Session>;
  abort(): Promise<void>;
  getSteps(): Step[];
  addStep(step: Omit<Step, "id" | "timestamp">): void;
}
```

### CrawlOptions

```typescript
interface CrawlOptions {
  /** Maximum crawl depth (0 = only the given URL, default: 2) */
  maxDepth?: number;
  /** Maximum number of pages to visit (default: 20) */
  maxPages?: number;
  /** Page load timeout in milliseconds (default: 10000) */
  pageTimeout?: number;
  /** Only follow same-origin links (default: true) */
  sameOrigin?: boolean;
  /** Callback fired after each page is crawled */
  onPageCrawled?: (url: string, formsFound: number) => void;
}
```

### Session

```typescript
interface Session {
  name: string;
  driver: string;
  driverConfig: Record<string, unknown>;
  steps: Step[];
  metadata?: {
    recordedAt?: string;
    version?: string;
    [key: string]: unknown;
  };
}
```

### Step

```typescript
interface Step {
  id: string;
  type: string;
  timestamp: number;
  [key: string]: unknown;
}
```

### RunResult

```typescript
interface RunResult {
  findings: Finding[];
  stepsExecuted: number;
  payloadsTested: number;
  duration: number;
  errors: string[];
}
```

## Complete Example

```typescript
import {
  DriverManager,
  PluginManager,
  parseProjectConfig,
} from "@vulcn/engine";
import browserDriver from "@vulcn/driver-browser";

async function runSecurityTest() {
  // Set up driver manager
  const drivers = new DriverManager();
  drivers.register(browserDriver);

  // Set up plugin manager from config
  const plugins = new PluginManager();
  const config = parseProjectConfig({
    payloads: { types: ["xss"] },
    detection: { xss: { dialogs: true, severity: "high" } },
  });
  await plugins.loadFromConfig(config);

  // Record a session
  const handle = await drivers.startRecording("browser", {
    startUrl: "https://vulnerable-app.example.com",
    browser: "chromium",
    headless: false,
  });

  console.log("Recording... press Ctrl+C when done");
  const session = await handle.stop();

  // Execute scan
  const result = await drivers.executeScan([session], plugins, {
    headless: true,
    onFinding: (finding) => {
      console.log(`🚨 ${finding.severity}: ${finding.title}`);
    },
  });

  console.log(`\nResults:`);
  console.log(`  Payloads: ${result.aggregate.payloadsTested}`);
  console.log(`  Findings: ${result.aggregate.findings.length}`);
  console.log(`  Duration: ${result.aggregate.duration}ms`);

  await plugins.destroy();
}

runSecurityTest().catch(console.error);
```

<Card title="Creating Drivers" icon="hammer" href="/drivers/creating-drivers">
  Learn how to build custom drivers
</Card>
