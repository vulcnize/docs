...
The report plugin generates branded, professional security reports at the end of each `vulcn run`. All formats are derived from the **canonical `VulcnReport` model** — a single source of truth built once via `buildReport()`, ensuring consistency across HTML, JSON, YAML, and SARIF outputs.
...
```bash
vulcn run session.vulcn.yml --report sarif --report-output ./reports
```
...
- **`ruleId`** — e.g. `VULCN-XSS` (derived from finding type)
- **`cwe`** — `{ id: 79, name: "Cross-site Scripting..." }` (from the CWE registry)
- **`securitySeverity`** — CVSS-like score (0.0–10.0)
- **`fingerprint`** — stable hash for deduplication across runs
- **`detectionMethod`** — `"active"` or `"passive"`
- **`passiveCategory`** — category ID when detection is passive
...
- **CWE Mappings** — each vulnerability type maps to its CWE ID (XSS → CWE-79, SQLi → CWE-89, etc.)
- **Security Severity Scores** — CVSS-like 0.0–10.0 scores for sorting in GitHub's Security tab
- **Deterministic Fingerprints** — prevents duplicate alerts across runs
- **Rule Deduplication** — one SARIF rule per vulnerability type, regardless of finding count
- **Invocation Metadata** — session name, duration, steps executed, and payloads tested are preserved
...
<Tip>
  Use `if: always()` on the upload step so SARIF results are uploaded even when
  vulnerabilities are found (exit code 1).
</Tip>
...
```bash
vulcn run session.vulcn.yml --report sarif || exit 1
```
...
- **🔒 Passive Analysis** — Grouped passive findings by category (Security Headers, Cookie Security, CORS, etc.) with remediation guidance
- **🃏 Expandable Findings** — Click to reveal description, payload, evidence, CWE reference, and metadata
