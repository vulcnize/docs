...
description: "Passive security scanner — detects misconfigurations without injecting payloads"
---
...
vulcn run session.vulcn.yml -p xss --passive

# Passive scanning with HTML report
vulcn run session.vulcn.yml --passive -r html
...
```
Session Replay → HTTP Response → Passive Analysis → Findings
                                  ├── Headers check
                                  ├── Cookie check
...
```
🔍 Passive scanner listening for network responses...
🛡️  Passive scan complete — 5 issues found
...
[MEDIUM] Cookie: Missing HttpOnly flag — session_id
  Type: security-misconfiguration
  Step: passive-scan
  URL: https://example.com/login
...
vulcn run session.vulcn.yml -p xss sqli --passive -r html

# Passive-only (still requires payloads for session replay)
vulcn run session.vulcn.yml --passive -r json
```

<Tip>
  The passive scanner runs alongside active testing with zero performance
  overhead — it only observes existing network traffic.
</Tip>
