...
<Info>
  **Multi-session scans**: When running a session directory with multiple
  ...
  aggregate report** containing findings from all sessions — not one report per
  session.
</Info>
...
if [ "$FINDINGS" -gt 0 ]; then
  echo "❌ $FINDINGS vulnerabilities found"
  exit 1
fi
```
...
<Card title="Plugin Hooks" icon="code" href="/plugins/hooks">
  ...
  hook (multi-session aggregate) — see all available hooks
</Card>
