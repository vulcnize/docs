...
vulcn run sessions/form.vulcn.yml --creds .vulcn/auth.enc -p xss -r html
```
...
### v2 Session Directory (recommended)

The v2 format uses a `.vulcn/` directory with a manifest:

```
my-scan.vulcn/
├── manifest.yml          # Scan config, session list, auth config
├── auth/
│   ├── config.yml        # Login strategy, indicators
│   └── state.enc         # Encrypted storageState
├── sessions/
│   ├── search-form.yml   # Individual session file
│   └── login-form.yml
└── requests/
    └── search-form.json  # HTTP metadata for Tier 1 scanning
```

### v1 Session File (legacy)

Single YAML file containing everything:
