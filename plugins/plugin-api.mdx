...
interface PluginContext {
  ...
  /** Shared findings collection (read-only view) */
  findings: Finding[];
...
interface Step {
  id: string;
  type: string; // e.g., "browser.navigate", "browser.input", "browser.click"
  url?: string;
  selector?: string;
  value?: string;
  injectable?: boolean;
...
```typescript
class PluginManager {
  /** Load plugins from flat VulcnProjectConfig (from .vulcn.yml) */
